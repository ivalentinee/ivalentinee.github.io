...
{% raw %}$$O = \{o | o ⊆ SO, i ≠ j, o_i ≠ o_j\}$${% endraw %}
...
#### Принцип замкнутости в отношении правил
Принцип замкнутости для правил политики авторизации можно сформулировать так: «Если для субъекта {% raw %}$$s_x ∈ S$${% endraw %}, действия {% raw %}$$a_y ∈ A$${% endraw %} и объекта {% raw %}$$o_z ∈ O$${% endraw %} не определено значение предиката p, то считаем, что высказывание {% raw %}$$p(\{s_x, a_y, o_z\})$${% endraw %} будет ложным». Этот принцип (или аксиома — как хотите) позволяет не определять правила для всех возможных случаев, а только специфицировать минимально необходимые.
